import { Button, FormControl, InputRightElement, NumberInput, NumberInputField } from "@chakra-ui/react";
import { BigNumber } from "@ethersproject/bignumber";
import { ChainId, CurrencyValue, NativeCurrency, useEtherBalance, useEthers } from "@usedapp/core";
...
export function WrapNativeCurrency() {
  ...
  const ethBalance = new CurrencyValue(new NativeCurrency('Native currency', 'BLANK', chainId ?? ChainId.Localhost), bignumBalance);
      ...
      <FormControl isInvalid={errors.name}>
        <NumberInput min={0} max={parseFloat(ethBalance.format())}>
          ...
          <InputRightElement width="4.5rem">
            <Button size="xs" onClick={() => setValue('wrap-amount', ethBalance.format())}>MAX</Button>
          </InputRightElement>
        </NumberInput>

        <Button type="submit" isLoading={isSubmitting}>Wrap native currency</Button>
